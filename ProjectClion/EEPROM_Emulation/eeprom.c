...
#else
  #define EEPROM_START_ADDRESS    (FLASH_END - 2*PAGE_SIZE - EEPROM_INDENT_PAGES_FROM_END*PAGE_SIZE + 1)
#endif
...
#define ERASED                  ((uint16_t)0x0000)     /* PAGE is empty */
#define RECEIVE_DATA            ((uint16_t)0x1111)     /* PAGE is marked to receive data */
#define VALID_PAGE              ((uint16_t)0x3333)     /* PAGE containing valid data */
...
static FLASH_Status EE_Format(void)
{
  ...
  if (FlashStatus != FLASH_COMPLETE)
  {
    ...
    // test
    FlashStatus = FLASH_ProgramHalfWord(PAGE0_BASE_ADDRESS, RECEIVE_DATA);
    if (FlashStatus != FLASH_COMPLETE)
    {
        return FlashStatus;
    }
...
//    if ((*(__IO uint32_t*)Address) == 0xFFFFFFFF)
    if ((*(__IO uint32_t*)Address) == 0x00000000)
    {
      /* Set variable data */
...
static FLASH_Status FLASH_ProgramHalfWord(uint32_t Address, uint16_t Data)
{
  ...
  else
  {
    ...
    if(status == FLASH_COMPLETE)
    {
        ...
        if ((Address & 0x3) == 0x0)
        {
            word = (*(__IO uint16_t *)(Address + 2) << 16);
            word |= Data;
